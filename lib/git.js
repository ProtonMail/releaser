...
const filterLocalCommits = ({ name = '', issues = [], match = '' }) => {
    return issues.length === 0 && name.includes(match);
};
...
const removeMatch = (match) => {
    ...
    return (commit) => {
        const name = commit.name.replace(regexp, '');
        return {
            ...commit,
            name
        };
 ...
 */
async function retrieve({ commits, externalLabels, localLabels, owner, repo, token }) {
