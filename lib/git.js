...
async function getLatestTagsFromGit({ cwd, count }) {
    const { stdout } = await exec(`git log --tags --simplify-by-decoration --pretty="format:%aI %D" | grep tag: | sort -r | head -${count}`, { cwd });
    return stdout;
}
 ...
 * @param {RegExp} tagFormat
 */
function tagParser(tagFormat) {
    return (input = '') => {
        const regex = /([^ ]+) tag: (.*)/;
